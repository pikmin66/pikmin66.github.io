...
function resetGame() {
    ...
    document.getElementById('player-trick-cards').innerHTML = '';
    document.getElementById('opponent-trick-cards').innerHTML = '';
...
function createCardElement(card) {
    ...
    if (card.image) {
        ...
        // If no image, display suit and rank
        const suitSymbols = {
            'Hearts': '♥',
            'Diamonds': '♦',
            'Clubs': '♣',
...
function playerTrickTakingPhase() {
    renderHands();
    showMessage('Drag a card to the play area to play it for the trick.');
    // Player will drag a card to the play area to play

    // Set up drop zone
    const playerTrickArea = document.getElementById('player-trick-cards');
    playerTrickArea.classList.add('drop-zone', 'flashing');
    playerTrickArea.addEventListener('dragover', (e) => {
        e.preventDefault();
    });
    playerTrickArea.addEventListener('drop', (e) => {
        e.preventDefault();
        if (currentPhase === 'Trick-Taking' && turnPlayer === 'Player') {
            playCard(draggedCardIndex);
            playerTrickArea.classList.remove('flashing');
        }
    });
}
...
function selectOpponentCard(playerCard = null) {
    ...
    let validCards = opponentHand;

    if (playerCard) {
        validCards = opponentHand.filter(card => card.suit === playerCard.suit);
        if (validCards.length === 0) {
            // If cannot follow suit, any card can be played
            validCards = opponentHand;
        }
    }
...
function displayTrick(playerCard, opponentCard) {
    const playerTrickArea = document.getElementById('player-trick-cards');
    const opponentTrickArea = document.getElementById('opponent-trick-cards');
...
function playerSummoningPhase() {
    showMessage('Drag creatures from your hand to the battlefield to summon them.');
    setupSummoning();
}
...
function setupSummoning() {
    ...
    playerBatteriesDiv.addEventListener('dragover', (e) => {
        e.preventDefault();
    });
    playerBatteriesDiv.addEventListener('drop', (e) => {
        e.preventDefault();
    ...
    // Allow setting inactives by dragging onto existing batteries
    playerField.forEach((battery, index) => {
        const batteryDiv = document.getElementById(`player-battery-${index}`);
        batteryDiv.classList.add('drop-zone', 'flashing');
        batteryDiv.addEventListener('dragover', (e) => {
            e.preventDefault();
        });
        batteryDiv.addEventListener('drop', (e) => {
            e.preventDefault();
            const card = playerHand[draggedCardIndex];
            if (battery.inactives.length < 2) {
                battery.inactives.push(card);
                playerHand.splice(draggedCardIndex, 1);
                showMessage(`You set ${card.name} under ${battery.active.name}.`);
                renderHands();
                renderBattlefield();
                actionsThisPhase++;
            } else {
                showMessage('This Battery already has two inactives.');
            }
        });
    });
}
...
function opponentSummonCreatures() {
    let summonableCards = opponentHand.filter(card => card.level && card.level <= opponentSP);
...
function createBatteryElement(battery, owner, index) {
    ...
    // Active Card
    const activeCardDiv = createCardElement(battery.active);
...
function showOptions(options) {
    ...
    options.forEach(option => {
        ...
        gameMessagesDiv.appendChild(button);
    });
}
